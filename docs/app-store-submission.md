...
> 提交，Apple 于 2026-09-01 14:24 拒绝，提交 ID 仍为
> `063f45f8-0cb8-4e3e-9dc9-dbed7bc5b151`。本次不再是 Guideline 5.6，具体为：
> Guideline 2.1(b) 要求说明商业模式，Guideline 1.5 指出当时填写的
> `https://qisw.top/rdesk/support` 不可访问。ASC 当前已保存新的技术支持 URL
> `https://qsw745.github.io/rdesk-support/`；2026-09-04 复核 `GET` / `HEAD`
> 均为 HTTP 200，外部探测在美国洛杉矶及多个欧洲节点也返回 200。
>
> 源码与依赖复核未发现 StoreKit、IAP、订阅、会员、支付或付费解锁流程；所有
> 远程连接、中继、云设备同步、画面查看/控制、文件传输、剪贴板和账号功能均免费，
> 账号创建免费且设备 ID 直连不要求账号。本轮不需要新二进制。回复前已使用审核
> 密码完成实体 Android 演示机全链路检查；22:49 已向 Apple 发送商业模式和新支持
> 网址说明，22:51 已重新提交，ASC 回读为“等待审核”。已选择审核通过后自动发布，
> 但当前不代表已批准或已公开上架。
...
>
> 下文已按 build 14 的实际能力更新，并与 2026-09-04 现场读取的 ASC 描述一致。
...
- **隐私政策 URL**：`https://qisw.top/rdesk/privacy` —— **已部署上线**
  ...
  > 与第三节的申报口径对应。
- **技术支持 URL**：`https://qsw745.github.io/rdesk-support/` —— **已在 ASC 保存并公开上线**

  这是针对 Guideline 1.5 的全球分发替代页。2026-09-04 用 Safari / iPhone
  User-Agent 复核 IPv4 `GET` / `HEAD` 均为 HTTP 200，DNS 同时提供 GitHub Pages
  的多组 IPv4 / IPv6 地址；外部节点复核包括美国洛杉矶及多个欧洲地区。

  页面内容涵盖：当前版本完全免费、快速连接、常见问题、真实平台能力边界、
  数据传输说明、隐私政策入口和联系邮箱。原页面
  `https://qisw.top/rdesk/support` 仍可作为历史部署保留，但不要再填入 ASC。
...
| 2.1.0 (13) | 2026-08-18 上传并完成出口合规；移除伪聊天与虚假快捷键入口，修正商店/审核/支持文案；重新提交后于 2026-08-28 第四次被 **Guideline 5.6** 拒绝 |
| 2.1.0 (14) | 2026-08-29 14:59 提交；缩短操作弹层，新增真实会话键盘、控制设置与网络状态。2026-09-01 被 **Guideline 2.1(b)**（需说明商业模式）和 **Guideline 1.5**（原支持网址不可达）拒绝；二进制本身未被指出新问题 |
...
- [x] 签名 IPA、ASC 上传、出口合规、build 14 选择、线上元数据和首次提交均已完成
- [x] ASC 技术支持 URL 已保存为 `https://qsw745.github.io/rdesk-support/`
- [x] 新支持页本机 `GET` / `HEAD` 与 9 个外部节点均返回 HTTP 200
- [x] 源码与依赖未发现 StoreKit、IAP、订阅、会员、支付或付费解锁流程
- [x] 实体 Android 演示机恢复后，已使用审核密码完整运行
      `scripts/check_review_host.sh`：进程、MediaProjection、无障碍、密码认证、
      在线心跳、实时画面和公共页面全部通过
- [x] 2026-09-04 22:49 已向 Apple 回复 2.1(b) 五个问题及新的 Support URL，
      并在 ASC 消息区回读确认
- [x] 2026-09-04 22:51 已重新提交 build 14，ASC 回读状态为“等待审核”

> 当前只能写成“等待审核”，不能写成“已批准”或“已公开上架”。审核期间应让
> 演示机保持专用副用户、充电、联网、MediaProjection、无障碍和无人值守在线。
...
- [x] 轮换演示机永久密码并用实体 iPhone 复测连接成功；回复 Apple 后重新提交，
      ASC 提交 ID `063f45f8-0cb8-4e3e-9dc9-dbed7bc5b151`，当时状态为“等待审核”

> 以上为 build 13 当时的历史记录；该状态后来已被第四次 Guideline 5.6 拒绝取代。
> 当前状态以本章 build 14 检查清单和文档开头的 2026-09-04 现场回读为准。
