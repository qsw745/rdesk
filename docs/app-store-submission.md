...
运行在本机 Android 模拟器上，AVD 名为 `RDeskDemo`（Android 14 / Pixel 5）。
选用模拟器而非实体手机，是为了给审核员一个**不含任何个人数据**的干净环境——
审核员连入后可以看到整个屏幕并操作系统 UI。

| 项 | 值 |
|---|---|
| 设备 ID | `927 251 902` |
| 永久密码 | `Review2026` |
| 无人值守 | 已开启「自动接受连接」，无需被控端手动确认 |
| 权限 | 录屏、无障碍、通知、电池优化白名单均已授予 |

已实测：iOS 端输入上述 ID 与密码后成功连接，画面实时推送（706×1440），
点击工具栏「返回」可使 Android 端退回桌面——证明远程操控确实下发到系统层。
