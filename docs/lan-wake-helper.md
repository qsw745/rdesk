...
当前程序可用于允许运行自定义程序的 Linux 路由器、NAS 或常开 Linux 主机，补充现有安卓助手。**尚未在用户的 AX3000T 原厂 2.0.28 固件上安装或验证，不能把二进制上传到“插件安装”当作小米插件。** 独立实验工具原 0.1.0 的 macOS 下载成品仅提供管理功能；App 2.2.3 的内嵌版本增加 Mac 发包支持，使用下文 App 入口，不需命令行。路由器仍需已有安装权限，没有原厂路由器一键安装入口。
...
## App 2.2.3：用家中 Mac 测试（不依赖安卓）

1. 在家中 Mac 安装完整 2.2.3，登录与 Windows 相同的 RDesk 账号，进入「远程开机」。
2. 开启「用这台 Mac 代发开机信号」，明确选择 Windows 所在的 LAN 接口。接口名/IP 仅在本地选择时显示，账号 token 通过进程标准输入传递，不进入参数、配置文件或日志。
3. 手机扫码配对 Windows，或在已配对电脑的配置中选择「家中 Mac」；已经绑定旧助手时，在「更多 → 更换助手」切换。
4. 进入「开机测试与诊断」。先自行保存工作、睡眠，再用手机蜂窝网络发起。分别记录云端接受、助手领取、发包回执、应用上线与现场物理结果。人工观察仅加入复制报告，不冒充自动检测。
5. Mac 需要留在家庭网络、保持供电和系统唤醒、RDesk 运行。锁屏可继续，合盖睡眠、退出或重启后不可依赖。App 不自动修改电源配置，不自动恢复助手；重新打开后需重新启用。

Mac 发包使用 IP_BOUND_IF 绑定所选接口，保留服务端短时授权、持久去重、断网退避和有界日志。App 退出后 stdin 管道断开，helper 取消正在等待的工作；切换账号和停用也会终止进程。更换网段/网卡时退出并要求重新选择，避免在其他网络继续发包。状态和去重按服务器/账号/助手隔离，权限 700/600；token 只在内存中。

此链路可以测试不依赖安卓的外网开机，但需要 Mac 在家运行；不能用其结果宣称 AX3000T 原厂固件已适配。Windows 自己关机后不能充当自己的助手。最终 24/48 小时验收仍需要用户实际执行。
...
4. 在助手设备执行 `rdesk-wake-helper run --state /etc/rdesk-wake-helper`。绑定接口需要 root 或 CAP_NET_RAW。随后在 RDesk 的远程开机设置中刷新助手，选择登记的名称。现有 App 列表按账号读取助手，不要求名称包含安卓；2.2.3 的 App 引导已同时说明安卓、Mac 和兼容 Linux 助手。
