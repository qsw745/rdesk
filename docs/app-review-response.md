...
- 「操作」弹层由接近全屏改为竖屏约 64%、横屏约 78%，顶部保留退出、仅观看、
  指针模式和旋转，长内容在弹层内部滚动。
- 会话底栏与「操作」页新增真实「电脑键盘」，提供「输入法 / 电脑键盘」切换。
  字母、数字、空格、删除和回车沿用现有真实输入链路；Esc、Tab、方向键以及
  全选、复制、粘贴、剪切、撤销、重做只在识别到 macOS 被控端时启用，并由
  macOS 主机通过 CGEvent 执行。Android 与未知系统会停用这些不支持的桌面键。
- 该入口不是恢复 build 13 删除的「我的 → 快捷键」静态说明页，也不宣称 Windows
  或 Android 支持 Cmd/Ctrl 等桌面组合键。
- 新增「控制设置」与「网络状态」。网络状态只显示当前能够取得的会话状态、
  画面延迟、帧率上限、画质、连接时长、远端系统和显示器，不伪造丢包率或带宽。
...
## 二、第三次拒绝后的回复 App 审核（build 13，实际发送稿）

> 已于 2026-08-18 在旧提交中回复 Apple；随后从被拒提交移除 App 版本，保留
> 已更新的元数据并将 build 13 重新加入新提交。
>
